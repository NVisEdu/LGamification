...
class UserRepository(RepositoryGettableAbstract):
    ...
    def get(self, ID: int):
        return cast( self.model, super().get(ID) )
    ...
    def create(self, username: str, password: str) -> "model":
        res = self.model(username, password)
        dbs.add(res)
        dbs.commit()
        return res

    def get_by_name(self, username: str) -> model:
        return dbs.execute(
            select(self.model)
